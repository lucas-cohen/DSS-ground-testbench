...
from control.default_dynamics import circle_motion, stay_at_position, swarm_circle, calibrate_directions, transform_frame
...
#            print(f"{self.name}: {name}={val}")
    ...
    def start_motion(self, dynamics, repeat=False):
        ...
        if delta_time >= self.update_freq:
            ...
            if abs(ea_raw) > np.pi:
                da = (ea_raw - np.sign(ea_raw)*2*np.pi)%(2*np.pi)
            else:
                da = ea_raw
            ...
            if self.debug:
                ...
                self.console_print("deltas : ", [round(dx, 3), round(dy,3)])
